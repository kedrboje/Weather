    ...
    static func info(fromJSON data: Data) -> RequestResult {
        do {
            ...
            for (index, _) in parsedResult.list.enumerated() {
                finalWeatherInfo.append(Wheather(temp: parsedResult.list[index].main!.temp!, pressure: parsedResult.list[index].main!.pressure!, humidity: parsedResult.list[index].main!.humidity!, id: parsedResult.city.id!, name: parsedResult.city.name!, description: nil, windSpeed: nil, dataTaken: nil))
            }
            print(parsedResult.list[0].main!.temp!, parsedResult.city.id!, parsedResult.city.name!)
